// a factory that we plug values into, and it gives us an oscillator with those values
export default class Osc {
  constructor(actx, type, frequency, detune, envelope, connection) {
    ...
    this.easing = 0.005;
    this.osc.start();
    this.start(); // call the start method below
  }
  start() {
    // start method of the actual _this_ object
    let { currentTime } = this.actx;
    this.gateGain.gain.cancelScheduledValues(currentTime); // cancels those values as soon as start is called
